//$('.carousel').carousel();
// MATCH DIV JAVASCRiPT

var dt = new Date();
var day = dt.getDate();
var hor = dt.getHours();

//considering a match day on NOV 2 and NOV 3
//the starting time is 7:30 pm
//the team logo will change 12 hours before the game

//console.log(day);
//console.log(hor)

document.getElementById("match1-team1").style.backgroundImage = "url('dc.png')";

document.getElementById("match1-team2").style.backgroundImage =
  "url('test.webp')";

document.getElementById("match2-team1").style.backgroundImage =
  "url('test.webp')";

document.getElementById("match2-team2").style.backgroundImage =
  "url('test.webp')";

document.getElementById("match3-team1").style.backgroundImage =
  "url('test.webp')";

document.getElementById("match3-team2").style.backgroundImage =
  "url('test.webp')";

document.getElementById("match4-team1").style.backgroundImage =
  "url('test.webp')";

document.getElementById("match4-team2").style.backgroundImage =
  "url('test.webp')";
